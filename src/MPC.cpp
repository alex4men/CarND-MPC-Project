 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 0; t < N - 1; t++) {
      ...
      // Add penalty for aggressive steering on high speeds
      fg[0] += 500*CppAD::pow(vars[delta_start + t] * vars[v_start+t], 2);
      // Penalize rapid CTE change -> smothes the MPC path
      fg[0] += 1000*CppAD::pow(vars[cte_start + t+1] - vars[cte_start + t], 2);
    }
    ...
    for (int t = 1; t < N; ++t) {
      ...
      // Take actuators values at time t-1 to compensate latency.
      if (t > 1) {
...
std::vector<double> MPC::Solve(const VectorXd &state, const VectorXd &coeffs) {
  ...
  // 0.1 seems to give more stability along with 100 milliseconds delay and dt = 0.1.
